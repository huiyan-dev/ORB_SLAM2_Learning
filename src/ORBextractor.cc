                           ...
                           iniThFAST(_iniThFAST),minThFAST(_minThFAST)
{
    ...
    //计算关键点的BRIEF描述子需要的pattern, 512个点对, static int bit_pattern_31_[256*4]
    //bit_pattern_31_中是 512 对 点对 相对中心点的坐标
    const int npoints = 512;
    const Point* pattern0 = (const Point*)bit_pattern_31_;
    std::copy(pattern0, pattern0 + npoints, std::back_inserter(pattern));   //复制给ORBextractor::pattern
                                                         ...
                                                         const int &maxX, const int &minY, const int &maxY, const int &N, const int &level)
{
    ...
    // -----------------5 现在一共得到了 nFeatures 个区域, 保留区域内最好的一个关键点(pKP.response最大的点)-----------------------
...
void ExtractorNode::DivideNode(ExtractorNode &n1, ExtractorNode &n2, ExtractorNode &n3, ExtractorNode &n4)
{
    ...
    for(size_t i=0;i<vKeys.size();i++)  //将所有关键点分配到四个子结点当中
    {
...
int main()
{
    ...
    //获得提取的特征点总数
    int sum_features_extract = 0;
    for(int i = 0; i < orb_extractor.nLevels; i++)
    {
        sum_features_extract += allKeyPoints[i].size();
    }
    //std::cout<<sum_features_extract<<" features have extracted"<<std::endl;
    ...
    //描述子计算
    Mat descriptors;
    descriptors.create(sum_features_extract, 32, CV_8U);
    vector<KeyPoint> _keyPoints;
    _keyPoints.reserve(sum_features_extract);

    int offset = 0;
    for(int level = 0; level < orb_extractor.nLevels; ++level)
    {
        ...
        //对图像高斯模糊,
        Mat tempMat = orb_extractor.mvImagePyramid[level].clone();
        GaussianBlur(tempMat, tempMat, Size(7, 7), 2, 2, BORDER_REFLECT_101);

        //计算描述子
        Mat desc = descriptors.rowRange(offset, offset + nkeyPointsLevel);
        ORB_SLAM2::computeDescriptors(tempMat, keyPoints, desc, orb_extractor.pattern);
        ...
        if (level != 0)
        {
            float scale = orb_extractor.mvScaleFactor[level]; //getScale(level, firstLevel, scaleFactor);
            for (vector<KeyPoint>::iterator keypoint = keyPoints.begin(),
                         keypointEnd = keyPoints.end(); keypoint != keypointEnd; ++keypoint)
                keypoint->pt *= scale;
        }
    ...
    std::cout<<"Descriptors information:"<<std::endl;
    for(int i = 0; i < 5; i++)
    {
        cout<<"keyPoints["<<i<<"].angle = "<<_keyPoints[i].angle<<std::endl;
    }
    return 0;
}